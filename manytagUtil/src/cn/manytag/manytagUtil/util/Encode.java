...
public class Encode {
	 ...
	 */
	public static BufferedImage encodeBarcode(String contents, int width, int height) throws WriterException {
	...
	public static BufferedImage scale(String imgPath, int width, int height, boolean hasFiller) {
		...
		if (hasFiller) {// 补白  
			...
			if (width == finalImage.getWidth(null)) {
				graphic.drawImage(finalImage, 0, (height - finalImage.getHeight(null)) / 2, finalImage.getWidth(null),
						finalImage.getHeight(null), Color.white, null);
			} else {
				graphic.drawImage(finalImage, (width - finalImage.getWidth(null)) / 2, 0, finalImage.getWidth(null),
						finalImage.getHeight(null), Color.white, null);
				graphic.dispose();
				finalImage = image;
			}
